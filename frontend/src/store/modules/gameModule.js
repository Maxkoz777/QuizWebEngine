...
export default {
  ...
  actions: {
    async validateQuizPin(context, quizId) {
      try {
        ...
        for (let question in response.data.questions) {
          let questionData = await context.dispatch('fetchQuestion', response.data.questions[question].questionId)
    ...
    async incrementQuestionNumber(context) {
      ...
      const question = context.getters.questions[context.getters.currentQuestionNumber]
      question ? await context.commit('setCurrentQuestion', question) : context.commit('setIsFinished', true)
    },
    async validateAnswer(context, answerId) {
      const correctAnswerData = await axios.get(`question/${context.getters.currentQuestion.questionId}/correctAnswer`)
